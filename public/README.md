...
1. **TCP Layer** — `net` opens a socket and buffers raw bytes until the full request arrives
2. **HTTP Parsing** — splits raw text into method, path, headers, and body
3. **Middleware Chain** — runs registered `app.use()` functions in order before the route
4. **Router** — matches the request against registered routes using regex
5. **Response Builder** — formats a valid HTTP/1.1 response and writes it to the socket
6. **Static Files** — streams files from disk with the correct MIME type
